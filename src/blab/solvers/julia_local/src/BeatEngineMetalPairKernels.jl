# Pair-owned regular Galerkin kernels over element-colored pair blocks. Each
# thread integrates one (test, trial) element pair once and scatters its 3x1
# and 3x3 blocks; the colorings guarantee that no two pairs in one launch share
# a P1 row or column, so the scatter needs no atomics and stays deterministic.
# This is the ROCm backend's production kernel, ported one for one.

@inline function _metal_pair_elements(
    color_elements,
    linear_index,
    test_start,
    test_count,
    ...
    return Int(color_elements[test_start + test_offset]), Int(color_elements[trial_start + trial_offset])
end
...
function _metal_regular_slp_adjoint_dlp_pairs_kernel!(
    single_layer,
    ...
    test_index, trial_index = _metal_pair_elements(
        color_elements,
        linear_index,
        Int(test_start),
        Int(test_count),
...
@inline function _metal_pair_curl_products(
    curls,
    test_index,
    trial_index,
    face_count,
...
function _metal_regular_hyp_pairs_kernel!(
    hypersingular,
    face_vertices,
    normals,
    areas,
...
function _launch_metal_colored_pair_kernels!(
    operators,
    ...
    for test_color in 1:color_count
        ...
        for trial_color in 1:color_count
            ...
            _metal_launch(
                _metal_regular_slp_adjoint_dlp_pairs_kernel!,
                pair_count,
            ...
            _metal_launch(
                _metal_regular_hyp_pairs_kernel!,
                pair_count,
...
function _launch_metal_regular_pair_kernels!(operators, cache::MetalRegularAssemblyCache, k)
    return _launch_metal_colored_pair_kernels!(
        operators,
        cache,
        k,
...
function _launch_metal_symmetry_regular_pair_kernels!(
    operators,
    cache::MetalRegularAssemblyCache,
