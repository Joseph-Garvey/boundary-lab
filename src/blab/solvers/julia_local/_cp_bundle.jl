...
# Load the engine from its bundle package when the active project has one.
# The bundle is the same sources, but a package's native code is cached in a
# pkgimage, so a worker that starts against an instantiated environment does
# not recompile the engine before it can accept a job. Falling back to the
# direct include keeps every checkout that has not been instantiated working
# exactly as before, at the old cost.
...
const BEAT_ENGINE_BUNDLED = try
    @eval using $BEAT_ENGINE_BUNDLE_NAME.BeatEngineCore
    true
catch
    false
end

if !BEAT_ENGINE_BUNDLED
...
function worker_loop()
    Printf.@printf(stderr, "CP engine %.3f | defs %.3f | entry-compile %.3f\n", _CP_T1-_CP_T0, _CP_T2-_CP_T1, time()-_CP_T2)
